...
`.github/workflows/ci.yml` owns the read-only `Quality gate`, responding to `pull_request` and `workflow_dispatch` but not to tag pushes. This fork carries no change-scope classifier and no trusted PR-smoke coordinator: the gate is a single job that always runs the full suite (`go test ./...`, `-race`, `go vet`, build, packaging and formatting) with `contents: read`, so no per-path skip decision has to be trusted. It never checks out or executes anything from a pull request beyond the read-only checkout the runner already performs.

`.github/workflows/platform-smoke.yml` and `.github/workflows/container-smoke.yml` remain as manual workers, but nothing dispatches them automatically in this fork. `platform-smoke.yml` additionally owns the manual `native_evidence` stage, which is the only entry point for the six-platform native evidence matrix. Ordinary branch and `main` pushes run no CI. Stable `vX.Y.Z` tag pushes run only `release.yml` (the Quality gate does not respond to tags, so Release solely owns the tag gates); Release itself performs the formal six-platform tests/builds and two-platform container verification. Browser/native evidence remain explicit maintenance workflows. Real Pixiv/FANBOX SDK E2E does not enter regular PR CI; only the tag-publishing `release.yml` runs the credential-free SDK E2E contract gate after validate, and real SDK E2E is still accepted independently on release-prep in an authorized environment.
...
Release verification now favors behavior contracts over a second workflow-policy implementation. `tools/platformmatrix` validates the shared platform registry, while `tools/release` owns reusable release trust and artifact checks: immutable tag/default-branch ancestry, published Release state, Release-run handoff identity, exact archive/container sets, and checksums. This fork has no publisher workflows, so there is no channel-specific credential, permission or recovery path to audit here; `tools/release` still owns every reusable handoff and artifact check the Release run itself performs.
...
This fork carries no rehearsal workflow, so reproduce the rehearsal locally instead: on an **already public, non-draft, non-prerelease** stable Release tag, run the `homebrewformula render` and staging-tap install chain described below. Confirm the input is a `v`-prefixed SemVer and that the GitHub Release's tag matches it; then download only that Release's published `checksums.txt`, render the `pixiv-cli` staging formula, and run a real local staging-tap install on the production-identical runners for macOS Intel/arm64 and Linux amd64/arm64.

This is a read-only rehearsal: it needs no `release` Environment, secret, tag checkout, or Release/asset editing, and it must not clone, commit or push the Homebrew tap. The Linux branch installs the read-only-mounted local staging formula in a fixed-digest, short-lived Homebrew container; macOS keeps the native ordinary install command.
It reproduces the Homebrew install chain before a formal release, and **does not replace** formal tag publishing, signed Releases, tap deployment or post-publish install acceptance. The quality gate keeps behavior-level formula and artifact checks rather than a second workflow-policy implementation.

The formal release must still be blocked by a formal tag, a signed GitHub Release and subsequent install acceptance. The complete six-target staticlib/manifest and real native artifact evidence must have been collected and backfilled under control (see the "Rust ugoira staticlib" section); the protected `release` Environment, the production signing private key and the public repository must also be configured, but these preconditions themselves do not mean that a Release/tap has been created or that the install path has been accepted.
...
The Homebrew tap is an independent publishing surface: stable uses `pixiv-cli`, pre-release uses `pixiv-cli-beta`, and both install `pixiv` and conflict with each other. This fork has no automated publisher for it, so any tap write is a manual maintainer action that must verify the published Release handoff and archive checksums before rendering and pushing the formula. Any deploy key used for that manual step must never enter source, logs, or artifacts. `release-approval` remains the single final approval for the Release itself.
...
A successfully completed `Release` workflow triggers no downstream publishers in this fork. The Release handoff still binds the original run, tag, commit, and prepared artifact identities; it is not merely a tag file. Anyone consuming it manually must revalidate the identity and the required artifacts first, and must not infer a version from a branch name or substitute later `main` content.
